...
def run_refresh(limit: int) -> RefreshResult:
    ...
    with NeonStore(settings.database_url) as store:
        ...
        print(f"中文日志：Nintendo 追踪刷新找到 {len(rows)} 个待刷新游戏，limit={limit}", flush=True)
        for row in rows:
        ...
        record_refresh_source_status(store, "nintendo", job.job_id, status, error_message)
        print(
            f"中文日志：Nintendo 追踪刷新完成，状态={status}，处理={job.processed_count}，失败={job.error_count}",
            flush=True,
        )
    ...
    with store.conn.cursor() as cur:
        cur.execute(
            ...
            tracked AS (
              ...
              WHERE COALESCE(sl.url, g.official_url) IS NOT NULL
