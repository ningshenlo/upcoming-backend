...
from collectors.xbox import fetch_api_context, fetch_product_payload, parse_products_payload
...
def run_refresh(limit: int) -> RefreshResult:
    ...
    if not settings.database_url:
        raise RuntimeError("DATABASE_URL is required for Xbox tracked refresh")

    context = fetch_api_context(settings.user_agent)
    with NeonStore(settings.database_url) as store:
        job = start_job(
            ...
            {
                "collectors": ["xbox"],
                "mode": "xbox_tracked_refresh",
                "limit": limit,
                "startedAt": datetime.now(timezone.utc).isoformat(),
            },
        )
        rows = _tracked_xbox_rows(store, limit)
        print(f"中文日志：Xbox 追踪刷新找到 {len(rows)} 个待刷新游戏，limit={limit}", flush=True)
        for row in rows:
            product_id = row["product_id"]
            try:
                payload = fetch_product_payload([product_id], settings.user_agent, context=context)
                game = _matching_game(parse_products_payload(payload), row)
                ...
                job.error_count += 1
                job.errors.append({"productId": product_id, "message": str(exc)})
                try:
                    store.conn.rollback()
                except Exception:
                    store.reconnect()
        ...
        record_refresh_source_status(store, "xbox", job.job_id, status, error_message)
        print(
            f"中文日志：Xbox 追踪刷新完成，状态={status}，处理={job.processed_count}，失败={job.error_count}",
            flush=True,
        )
        return RefreshResult(status=status, processed_count=job.processed_count, failed_count=job.error_count)


def _tracked_xbox_rows(store: NeonStore, limit: int) -> list[dict[str, Any]]:
    ...
    with store.conn.cursor() as cur:
        cur.execute(
            ...
            LIMIT %s
            """,
            (["xbox-series", "xbox-one", "pc", "xbox-game-pass"], limit),
        )
...
def _matching_game(games: list[CollectedGame], row: dict[str, Any]) -> CollectedGame | None:
    ...
    for game in games:
        if product_id and str(game.external_ids.get("xboxProductId") or "") == product_id:
            return game
        ...
        for link in game.store_links:
            if product_id and str(link.product_id or "") == product_id:
                return game
            if sku_id and str(link.sku_id or "") == sku_id:
                return game
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh tracked upcoming Xbox games and record game_updates changes.")
...
def main() -> int:
    ...
    print(
        "xbox-tracked-refresh completed: "
