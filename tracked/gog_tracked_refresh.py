...
from collectors.gog import fetch_game_payload, parse_game_payload
...
def run_refresh(limit: int) -> RefreshResult:
    ...
    if not settings.database_url:
        raise RuntimeError("DATABASE_URL is required for GOG tracked refresh")
    ...
    with NeonStore(settings.database_url) as store:
        job = start_job(
            ...
            {
                "collectors": ["gog"],
                "mode": "gog_tracked_refresh",
                "limit": limit,
                "startedAt": datetime.now(timezone.utc).isoformat(),
            },
        )
        rows = _tracked_gog_rows(store, limit)
        print(f"中文日志：GOG 追踪刷新找到 {len(rows)} 个待刷新游戏，limit={limit}", flush=True)
        for row in rows:
        ...
        print(
            f"中文日志：GOG 追踪刷新完成，状态={status}，处理={job.processed_count}，失败={job.error_count}",
            flush=True,
        )
        return RefreshResult(status=status, processed_count=job.processed_count, failed_count=job.error_count)


def _tracked_gog_rows(store: NeonStore, limit: int) -> list[dict[str, Any]]:
...
def _matching_game(games: list[CollectedGame], row: dict[str, Any]) -> CollectedGame | None:
    ...
    for game in games:
        if product_id and str(game.external_ids.get("gogProductId") or "") == product_id:
            return game
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh tracked upcoming GOG games and record game_updates changes.")
