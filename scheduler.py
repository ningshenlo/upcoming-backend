...
def build_jobs(env: Mapping[str, str] | None = None, python_executable: str | None = None) -> list[ScheduledJob]:
    ...
    return [
        ...
        ScheduledJob(
            key="igdb-discovery-sync",
            job_type="igdb_discovery_sync",
            mode=None,
            command=(
                python,
                "igdb_discovery_sync.py",
                "--limit",
                str(_int_env(values, "IGDB_DISCOVERY_LIMIT", 50)),
            ),
            interval_minutes=_int_env(values, "IGDB_DISCOVERY_SYNC_INTERVAL_MINUTES", 1440),
            enabled=_bool_env(values, "IGDB_DISCOVERY_SYNC_ENABLED", True),
        ),
        ...
        ScheduledJob(
            ...
            command=(
                ...
                "--discover-limit",
                str(_int_env(values, "HOT_TRACKER_DISCOVERY_LIMIT", 80)),
                "--refresh-limit",
                str(_int_env(values, "HOT_TRACKER_REFRESH_LIMIT", 500)),
                "--rediscovery-days",
                str(_int_env(values, "HOT_TRACKER_REDISCOVERY_DAYS", 7)),
            ),
            interval_minutes=_int_env(values, "HOT_TRACKER_INTERVAL_MINUTES", 1440),
